"""Aggregate corrected PSMI metrics and checkpoint hashes across fixed seeds."""
...
def collect_rows(seeds: list[int]) -> list[dict]:
    ...
    for seed in seeds:
        locations = {
            "supervised": PROJECT_ROOT / "results" / "main_benchmark" / "corrected_v2" / f"seed{seed}" / "stage1_supervised",
            "physics": PROJECT_ROOT / "results" / "main_benchmark" / "corrected_v2" / f"seed{seed}" / "stage2_physics",
            "expanded": PROJECT_ROOT / "results" / "transfer_evaluation" / "expanded_lle" / "corrected_v2" / f"seed{seed}",
        }
        for stage, run_dir in locations.items():
            ...
            if not metrics_path.is_file() or not checkpoint_path.is_file():
                raise FileNotFoundError(f"Incomplete corrected run: {run_dir}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "results" / "summaries" / "corrected_v2_multiseed",
    )
