...
def train_or_load(train_df, val_df, test_df):
    ...
    if ckpt_path and os.path.isfile(ckpt_path):
        ...
        print(f"  - Best epoch from ckpt: {ckpt.get('best_epoch', 'N/A')}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0:
            ...
            # ---- 关键：best checkpoint 按 early_stop_metric 保存（指令 2）----
            monitor_key = early_stop_metric  # 你前面已根据 mode 设好了
            monitor_val = val_m.get(monitor_key, float("nan"))

            # 这些指标一般都是越小越好；如果你后续加"越大越好"的指标再单独处理
            if not np.isnan(monitor_val):
                improved = False
                if early_stop_metric == "r2":
                    improved = monitor_val > best_monitor_value
                else:
                    improved = monitor_val < best_monitor_value

                if improved:
                    best_monitor_value = float(monitor_val)
            ...
            if use_early_stop:
                ...
                if not np.isnan(current_monitor):
                    # 判断是否改善
                    improved = False
                    if early_stop_metric == "r2":  # R² 越大越好
                        if current_monitor - best_monitor_value > early_stop_min_delta:
                            improved = True
                            best_monitor_value = current_monitor
                    else:  # 其他指标（mse/mae/rmse/mu_res_mae/tpd_viol_rate 等）都是越小越好
                        if best_monitor_value - current_monitor > early_stop_min_delta:
                            improved = True
                            best_monitor_value = current_monitor
