# -*- coding: utf-8 -*-
"""Evaluate PSMI checkpoints and generate explanation artifacts."""
...
def save_barh(names: List[str], vals: np.ndarray, out_path: str, title: str, topk: int = 30, font_scale: float = 1.0) -> None:
    ...
    ax.set_yticklabels(sel_names, fontsize=int(13*font_scale))
    ax.set_xlabel('Importance', fontsize=int(13*font_scale))
    ax.set_title(title, fontsize=int(14*font_scale), pad=15)
    ...
    ax.tick_params(axis='x', labelsize=int(13*font_scale))
    plt.tight_layout()
...
def plot_shap_style_importance(
    ...
    ax_main.set_yticklabels(top_features[::-1], fontsize=12)
    ax_main.set_xlabel('SHAP value (impact on model output)', fontsize=13)
    ...
    for plot_idx, feat_idx in enumerate(dependence_indices):
        ...
        ax.set_xlabel(feature_names[feat_idx], fontsize=12)
        ax.set_ylabel('Impact on Model Output', fontsize=12)
...
def visualize_all_test_groups_big(model: torch.nn.Module, T_scaler,
    ...
    print("[OK] Saved test ternary PDF:", pdf_path)
    print("[OK] Saved per-group PNGs:", png_dir)
...
def _objective(pred: torch.Tensor, y: Optional[torch.Tensor], objective: str, target: str) -> torch.Tensor:
    """Compute the selected explanation objective."""
...
def explain_integrated_gradients_one_sample(
    ...
    """Explain integrated gradients one sample."""
...
def graph_explainer_one_sample(
    ...
    """Process graph explainer one sample."""
    x0 = batch_to_device(x, device)
...
def collect_shap_style_data(
    ...
    result = {}
    n_synth_samples = min(max_samples, 100)  
...
def explain_system_and_plot(
    ...
    ds = GraphLLEDataset(
        ...
        g_cache=g_cache,
        fg_cache=fg_cache,
        scalar_dim=int(getattr(loaded.model, "scalar_dim", getattr(C, "SCALAR_DIM", 3))),
    ...
    for x, y in loader:
        if explain == "ig":
            ...
            if "mix_edge_mask" in masks:
                ...
                mix_edge_sum = v if mix_edge_sum is None else mix_edge_sum + v
            # fg masks or fg_feat (for saliency)
            if "fg_mask" in masks:
                v = np.asarray(masks["fg_mask"]).reshape(-1, masks["fg_mask"].shape[-1]).mean(axis=0).astype(np.float64)
                fg_imp_sum = v if fg_imp_sum is None else fg_imp_sum + v
            elif "fg_feat" in masks:
                # Support saliency method which returns fg_feat instead of fg_mask
                v = np.asarray(masks["fg_feat"]).astype(np.float64)
                fg_imp_sum = v if fg_imp_sum is None else fg_imp_sum + v
            else:
                # fg1/2/3 masks or feats
                v_all = None
                ...
                if v_all is None:
                    # Try fg1_feat/fg2_feat/fg3_feat for saliency method
                    for k in ["fg1_feat", "fg2_feat", "fg3_feat"]:
                        if k in masks:
                            v = np.asarray(masks[k]).astype(np.float64)
                            v_all = v if v_all is None else (v_all + v)
    ...
    if fg_imp_sum is not None:
        ...
        save_barh(fg_names, fg_imp_sum, os.path.join(out_dir, "fg_importance_topk.png"), title, topk=actual_topk, font_scale=args.font_scale)
...
def build_eval_dataloaders(
    ...
    if split_mode == "random":
        train_df, val_df, test_df = split_by_system(df_raw, train_ratio=0.8, val_ratio=0.1, seed=int(seed))
    else:
        train_df, val_df, test_df = stratified_split_by_system(
            df_raw, train_ratio=0.8, val_ratio=0.1, seed=int(seed), n_bins=8, min_bin_size=3
        )
    ...
    if use_graph:
        ...
        smiles_all = []
        for col in ["smiles1", "smiles2", "smiles3"]:
            smiles_all += df_raw[col].dropna().astype(str).tolist()
        g_cache.build_from_smiles(smiles_all)

        scalar_dim = int(getattr(loaded.model, "scalar_dim", getattr(C, "SCALAR_DIM", 3)))
        train_set = GraphLLEDataset(train_df, loaded.T_scaler, g_cache=g_cache, fg_cache=fg_cache, scalar_dim=scalar_dim, precompute_scalars=True)
        val_set = GraphLLEDataset(val_df, loaded.T_scaler, g_cache=g_cache, fg_cache=fg_cache, scalar_dim=scalar_dim, precompute_scalars=True)
        test_set = GraphLLEDataset(test_df, loaded.T_scaler, g_cache=g_cache, fg_cache=fg_cache, scalar_dim=scalar_dim, precompute_scalars=True)
...
def run_mode_test(args: argparse.Namespace) -> None:
    ...
    print("[1] Load Excel ...")
    df_raw, df_aug = load_and_prepare_excel(C.EXCEL_PATH, C.MIN_POINTS_PER_GROUP, False)
    ...
    if args.explain and args.explain.lower().strip() != "none":
        ...
        if stats_df is not None:
            ...
            print("[OK] mix-edge input stats saved:", stats_path)
        ...
        for tgt in targets:
            ...
            for gi in ["g1", "g2", "g3"]:
                if f"{gi}_node_feat" in agg:
                    ...
                    save_barh(a_names, agg[f"{gi}_node_feat"], os.path.join(exp_dir, f"{gi}_atom_feature_topk.png"),
                              f"{gi.upper()} atom-feature importance", topk=int(args.topk), font_scale=args.font_scale)
                ...
                if f"{gi}_edge_feat" in agg:
                    ...
                    save_barh(b_names, agg[f"{gi}_edge_feat"], os.path.join(exp_dir, f"{gi}_bond_feature_topk.png"),
                              f"{gi.upper()} bond-feature importance", topk=int(args.topk), font_scale=args.font_scale)
                ...
                if f"{gi}_glob_feat" in agg:
                    ...
                    save_barh(g_names, agg[f"{gi}_glob_feat"], os.path.join(exp_dir, f"{gi}_global_feature_topk.png"),
                              f"{gi.upper()} global-feature importance", topk=min(int(args.topk), len(g_names)), font_scale=args.font_scale)
            ...
            for key, suffix, title_suffix in [
                ...
                if key in agg:
                    ...
                    save_barh(me_names, agg[key],
                              ...
                              topk=min(int(args.topk), len(me_names)), font_scale=args.font_scale)
            ...
            try:
                ...
                if "mix_edge_gradx_feat" in shap_data:
                    ...
                    if data['importance'].shape[0] > 0 and len(data['importance'].shape) == 2:
                        ...
                        print("  [OK] SHAP-style plot saved: mix_edge_shap_style.png")
            except Exception as e:
                import traceback
                print(f"  [WARN] SHAP-style plot failed: {e}")
            ...
            if "mix_node_feat" in agg:
                ...
                save_barh(mn_names, mn, os.path.join(sub_dir, "mix_node_feature_topk.png"),
                          "Mixture node-feature importance", topk=min(int(args.topk), len(mn_names)), font_scale=args.font_scale)
            ...
            if "fg_feat" in agg:
                ...
                save_barh(fg_names, fg_imp, os.path.join(sub_dir, "fg_importance_topk.png"),
                          "FG importance", topk=int(args.topk), font_scale=args.font_scale)
            ...
            try:
                ...
                if len(img_paths) == 9:
                    ...
                    print(f"[OK] 3x3 grid composed: {grid_path}")
            except Exception as e:
                print(f"[WARN] Grid composition failed: {repr(e)}")

        print("[OK] explain outputs:", exp_dir)
...
def run_mode_system(args: argparse.Namespace) -> None:
    ...
    if args.explain and args.explain.lower().strip() != "none":
        ...
        for tgt in targets:
            ...
            print("[OK] system explain outputs:", exp_dir)
