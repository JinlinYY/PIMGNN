# -*- coding: utf-8 -*-
"""Provide molecular featurization, graph construction, scaling, and batching utilities."""
...
def renorm3(a: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    """Normalize a three-component composition."""
...
def temperature_scalar_value(
    temperature: Any,
    mode: str = "linear_quadratic",
    reference_k: float = 500.0,
) -> np.ndarray:
    """Return the raw scalar temperature variable used before standardization.

    The original model standardizes temperature in kelvin.  The reciprocal
    ablation instead standardizes ``T_ref / T``; multiplication by ``T_ref``
    keeps values numerically well scaled and does not alter the information in
    the thermodynamically motivated ``1/T`` variable.
    """
    values = np.asarray(temperature, dtype=np.float32)
    key = str(mode).strip().lower()
    if key in {"inverse", "reciprocal", "inv_t", "1/t"}:
        if np.any(values <= 0):
            raise ValueError("Absolute temperature must be positive for reciprocal encoding.")
        return (float(reference_k) / values).astype(np.float32)
    if key in {"linear_quadratic", "polynomial", "poly", "t_t2"}:
        return values.astype(np.float32)
    raise ValueError(f"Unsupported temperature encoding: {mode!r}")


def temperature_edge_basis(
    temperature_k: float,
    mode: str = "linear_quadratic",
    reference_k: float = 500.0,
) -> np.ndarray:
    """Build the two temperature terms appended to mixture-edge features."""
    temperature_k = float(temperature_k)
    reference_k = float(reference_k)
    if temperature_k <= 0 or reference_k <= 0:
        raise ValueError("Temperature and reference temperature must be positive.")
    key = str(mode).strip().lower()
    if key in {"inverse", "reciprocal", "inv_t", "1/t"}:
        value = reference_k / temperature_k
    elif key in {"linear_quadratic", "polynomial", "poly", "t_t2"}:
        value = temperature_k / reference_k
    else:
        raise ValueError(f"Unsupported temperature encoding: {mode!r}")
...
def assign_t_by_pca(group: pd.DataFrame) -> pd.DataFrame:
    """Assign the phase-path coordinate using PCA ordering."""
...
def safe_group_apply_t(df: pd.DataFrame) -> pd.DataFrame:
    """Assign phase-path coordinates within each system-temperature group."""
...
def mol_global_features(mol: Chem.Mol) -> np.ndarray:
    """Compute global molecular descriptors."""
...
def batch_graphs(graphs: List[GraphDict]) -> GraphDict:
    """Batch molecular graph dictionaries."""
    import torch
...
def batch_to_device(obj: Any, device: str) -> Any:
    """Move a nested batch to the target device."""
    import torch
...
def smiles_to_3d_package(smiles: str, seed: int = 0) -> Dict[str, Any]:
    """Build cached three-dimensional molecular features from SMILES."""
    smi = canonicalize_smiles(smiles)
...
def pair_interaction_features_3d(
    pkg_a: Dict[str, Any],
    pkg_b: Dict[str, Any],
    T: float,
    seed: int = 0,
    temperature_encoding: str = "linear_quadratic",
    temperature_reference_k: float = 500.0,
) -> np.ndarray:
    """Compute pairwise three-dimensional interaction features."""
    ...
    # Temperature basis.  Both alternatives retain two edge dimensions so the
    # comparison changes only the basis, not model size.
    T = float(T)
    T1, T2 = temperature_edge_basis(
        T,
...
def build_mixture_graph(
    ...
    **kwargs,  
    ...
    def _node_feat(smi: str) -> np.ndarray:
        ...
        f = g.astype(np.float32, copy=False)  
    ...
    def _get_pair_feat(pkg_a: Dict[str, Any], pkg_b: Dict[str, Any],
        ...
        if hit is not None:
            return hit
        feat = pair_interaction_features_3d(
            pkg_a,
            pkg_b,
            T=T_use,
            seed=_stable_int_seed(f"{seed}|pair|{tag}", seed),
            temperature_encoding=getattr(cfg, "TEMPERATURE_ENCODING", "linear_quadratic"),
            temperature_reference_k=float(getattr(cfg, "TEMPERATURE_REFERENCE_K", 500.0)),
        )
