# -*- coding: utf-8 -*-
"""Generate extended statistical figures from saved PSMI predictions."""
import os
...
def parity_plot_combined(df: pd.DataFrame, out_path: str):
    ...
    for ax, phase in zip(axes, ["E", "R"]):
        ...
        ax.set_xlabel(f"True composition", fontname="Arial", fontsize=base_fontsize)
        ax.set_ylabel(f"Predicted composition", fontname="Arial", fontsize=base_fontsize)
        phase_title = f"{phase_name} Phase"
        ax.set_title(f"{phase_title}", fontname="Arial", fontsize=base_fontsize)
        ax.set_aspect('equal')
        ...
        ax.text(
            ...
            va="top", ha="left",
            fontname="Arial", fontsize=base_fontsize,
            bbox=dict(boxstyle="round,pad=0.4", facecolor="white", 
                     alpha=0.92, linewidth=1.2, edgecolor=NATURE_COLORS['reference']),
        )
        
        leg = ax.legend(loc="lower right", frameon=True, fontsize=base_fontsize)
        leg.get_frame().set_edgecolor(NATURE_COLORS['reference'])
        leg.get_frame().set_linewidth(1.3)
        leg.get_frame().set_alpha(0.95)
    
    # fig.suptitle("Comparison of Predicted and Experimental Composition", y=0.98, fontname="Arial", fontsize=base_fontsize)
...
def plot_residual_vs_true_combined(df: pd.DataFrame, out_path: str, max_points: int = 8000, seed: int = 0):
    """Plot residual vs true combined."""
    fig, axes = plt.subplots(2, 3, figsize=(14.5, 8.5), sharex=True, sharey=False)
...
def plot_violin_or_box_by_category(
    ...
    """Plot violin or box by category."""
...
def plot_violin_combined_categories(
    ...
    """Plot violin combined categories."""
    categories = [
...
def plot_group_ternary_from_csv(
    ...
    fig.subplots_adjust(left=0.23, right=0.9, bottom=0.2, top=0.90)
    fig.suptitle(f"TEST | System {system_id} | T={T:.2f} K | n={len(g)}", y=0.985, fontname="Arial", fontsize=base_fontsize)
    ...
    leg = fig.legend(handles, labels, loc="upper left", bbox_to_anchor=(0.02, 0.88), frameon=True, ncol=1, fontsize=base_fontsize)
...
def main():
    ...
    if args.skip_ternary:
        print("[OK] Saved:", os.path.join(out_dir, "parity_combined.png"))
        if not args.skip_extra:
            print("[OK] Saved extra plots to:", os.path.join(out_dir, "extra_plots"))
        print("DONE:", out_dir)
        return
    ...
    if not args.skip_extra:
        print("[OK] Saved extra plots to:", os.path.join(out_dir, "extra_plots"))
    print("[OK] Saved ternary PNGs to:", ternary_dir)
    print("DONE:", out_dir)
