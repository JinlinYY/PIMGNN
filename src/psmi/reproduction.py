...
def evaluate_saved_checkpoint(
    ...
    *,
    ...
    if reference_predictions is None:
        reference_path = checkpoint_path.parent / "test_df_raw_pointwise_predictions.csv"
    else:
    ...
    report: Dict[str, Any] = {
        ...
        "feature_scalers": {
            ...
            "temperature_mean": float(temperature_scaler.mean),
            "temperature_standard_deviation": float(temperature_scaler.std),
            "pressure_mean": (
                float(pressure_scaler.mean) if pressure_scaler is not None else None
            ),
            "pressure_standard_deviation": (
                float(pressure_scaler.std) if pressure_scaler is not None else None
            ),
        },
        "partition_counts": {
            "all_rows": int(len(raw_frame)),
            "train_rows_before_augmentation": int(len(train_frame)),
            "validation_rows": int(len(validation_frame)),
            "test_rows": int(len(test_frame)),
            "train_systems": int(train_frame["system_id"].nunique()),
            "validation_systems": int(validation_frame["system_id"].nunique()),
            "test_systems": int(test_frame["system_id"].nunique()),
        },
        "metrics": metrics,
        "prediction_file": {
