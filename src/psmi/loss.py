...
from .ge_models import SUPPORTED_GE_MODELS, ge_mu_residual
...
class NRTLParamStore:
    ...
    def __init__(self, json_path: str, device: str = "cpu", dtype: torch.dtype = torch.float32):
        ...
        self.model = str(self.meta.get("model", "nrtl")).lower()
...
class MechanisticNRTLLoss(nn.Module):
    ...
    where L_sup is MSE on (E+R) compositions, and L_phy is a GE-model
    chemical-potential residual loss. The class name is retained for checkpoint
    and import compatibility with the original NRTL-only implementation.
    ...
    def __init__(
        ...
        self.ge_model = str(ge_model).lower()
        if self.ge_model not in SUPPORTED_GE_MODELS:
            raise ValueError(
                f"Unsupported GE model {self.ge_model!r}; choose from {SUPPORTED_GE_MODELS}"
            )
        self.w_eq = float(w_eq)
        self.w_gd = float(w_gd)
        self.w_stab = float(w_stab)
        if self.ge_model != "nrtl" and (self.w_gd > 0.0 or self.w_stab > 0.0):
            raise ValueError(
                "Gibbs-Duhem and TPD auxiliary penalties are NRTL-only; "
                "set w_gd=w_stab=0 for alternative GE models."
            )
        ...
        if self.store.model != self.ge_model:
            raise ValueError(
                f"Parameter-store model {self.store.model!r} does not match "
                f"requested GE model {self.ge_model!r}."
            )
