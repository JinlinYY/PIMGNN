...
def compute_metrics_from_csv(csv_path: str) -> Dict[str, float]:
    ...
    if 'target' not in df.columns or 'pred' not in df.columns:
        raise ValueError(f"CSV file must Contains 'target' and 'pred' column . current column : {list(df.columns)}")
    ...
    if len(y_true) == 0:
        raise ValueError(" None valid prediction and target ")
...
def recompute_all_metrics(results_dir: str, seeds: List[int] = [42, 123, 456, 789, 2024]) -> pd.DataFrame:
    ...
    print("="*80)
    print(" from CSV file Heavy new compute test metrics ")
    print("="*80)
    ...
    # Set the random seed.
    print("\n" + "="*80)
    print(" test predictions for each seed ")
    print("="*80)
    ...
    for seed in seeds:
        ...
        with open(summary_txt_path, 'w', encoding='utf-8') as f:
            ...
            if stats:
                ...
                if 'test_mae_format' in stats:
                    ...
                    if stats['test_mae_std'] < 0.001:
                        f.write(f" ( mean : {stats['test_mae_mean']:.6f}, standard deviation : {stats['test_mae_std']:.6f})\n")
                    elif stats['test_mae_std'] < 0.01:
                        f.write(f" ( mean : {stats['test_mae_mean']:.4f}, standard deviation : {stats['test_mae_std']:.4f})\n")
                    else:
                        f.write(f" ( mean : {stats['test_mae_mean']:.3f}, standard deviation : {stats['test_mae_std']:.3f})\n")
                if 'test_rmse_format' in stats:
                    f.write(f"RMSE: {stats['test_rmse_format']}\n")
                    if stats['test_rmse_std'] < 0.001:
                        f.write(f" ( mean : {stats['test_rmse_mean']:.6f}, standard deviation : {stats['test_rmse_std']:.6f})\n")
                    elif stats['test_rmse_std'] < 0.01:
                        f.write(f" ( mean : {stats['test_rmse_mean']:.4f}, standard deviation : {stats['test_rmse_std']:.4f})\n")
                    else:
                        f.write(f" ( mean : {stats['test_rmse_mean']:.3f}, standard deviation : {stats['test_rmse_std']:.3f})\n")
                if 'test_r2_format' in stats:
                    f.write(f"R²:   {stats['test_r2_format']}\n")
                    if stats['test_r2_std'] < 0.001:
                        f.write(f" ( mean : {stats['test_r2_mean']:.6f}, standard deviation : {stats['test_r2_std']:.6f})\n")
                    elif stats['test_r2_std'] < 0.01:
        ...
        print(f" This Seed test metrics :")
        # Set the random seed.
...
def main():
    ...
    parser = argparse.ArgumentParser(description=' from CSV file Heavy new compute metrics ')
