...
def format_metrics_detailed(metrics: Dict[str, float], prefix: str = "") -> str:
    ...
    lines = [
        ...
        f"{prefix}[R phase metrics ] MAE={metrics['mae_R']:.6f} RMSE={metrics['rmse_R']:.6f} R²={metrics['r2_R']:.6f}",
    ]
    return "\n".join(lines)
...
def train_mmgnn(
    ...
    print(f" number of training systems : {train_df['system_id'].nunique()}")
    print(f" number of validation systems : {val_df['system_id'].nunique()}")
    print(f" number of test systems : {test_df['system_id'].nunique()}")
    ...
    print(f" minimum early-stopping improvement : {min_delta}")
    print(f" checkpoint frequency : per {save_checkpoint_every} epoch")
    print(f" rest policy : per {rest_interval} epoch Break {rest_duration} seconds ({rest_duration/60:.1f} minutes )")
    if resume_from:
        print(f" resume training : {resume_from}")
    else:
    ...
    print(f" number of graph-neural-network layers : {num_layers}")
    print(f" Set2Set number of steps : {set2set_steps}")
    print(f" interpret after number of layers : {post_explain_layers}")
    print(f" Beta parameters : {beta}")
    print(f" interpret Controller Method : {explainer_method}")
    ...
    print(f" result directory : MMGNN/results")
    
    print("\n" + "=" * 100)
    print(" configuration parameters output complete , start Initial Training ...")
    print("=" * 100 + "\n")
    # ====================================
    ...
    if resume_from and os.path.exists(resume_from):
        ...
        if 'config' in checkpoint:
            ...
            print("=" * 100)
            print(f" checkpoint in found configuration information , will use checkpoint configuration :")
    ...
    if resume_from and os.path.exists(resume_from):
        ...
        print("=" * 100)
        print(f" resume training from a checkpoint : {resume_from}")
        print("=" * 100)
        print("=" * 100)
        print(f" resume training from a checkpoint : {resume_from}")
        print("=" * 100)
        ...
        if 'state_dict' in checkpoint:
            try:
                ...
                print("✓ model state loaded successfully ( partial match )")
    ...
    print(f" training epochs : {epochs}")
    if start_epoch > 1:
        print(f" from epoch {start_epoch} continue training ( Remaining {epochs - start_epoch + 1} round )")
    else:
        print(f" train from scratch ( total {epochs} round )")
    print(f" batch size : {batch_size}")
    print(f" learning rate : {lr}")
    print(f" weight decay : {weight_decay}")
    print(f" early-stopping patience : {patience}")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if early_stopping(val_metrics['mse']):
            ...
            print(f" waited {early_stopping.counter}/{early_stopping.patience} epoch without improvement ")
            break
    ...
    with open(metrics_txt_path, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("MMGNN Training results metrics \n")
        f.write("="*100 + "\n\n")
        ...
        f.write("="*100 + "\n")
        f.write(" per round training metrics Summary \n")
        f.write("="*100 + "\n\n")
        ...
        for i, epoch_num in enumerate(history['epoch']):
            ...
            f.write(f" training set - {format_metrics_detailed(train_metrics_dict, ' ')}\n")
            ...
            f.write(f" validation set - {format_metrics_detailed(val_metrics_dict, ' ')}\n")
        
        f.write("\n" + "="*100 + "\n")
        f.write(" best-model metrics ( Based on validation set MSE)\n")
        f.write("="*100 + "\n\n")
    ...
    print(f"\n result file saved :")
    print(f" - training history CSV: {os.path.join(out_dir, 'train_history.csv')}")
    print(f" - Training / validation results CSV: {os.path.join(results_dir, 'training_results.csv')}")
    print(f" - test-set results CSV: {os.path.join(results_dir, 'test_results.csv')}")
    print(f" - training metrics TXT: {os.path.join(results_dir, 'training_metrics.txt')}")
    print(f" - model checkpoint : {os.path.join(out_dir, 'mmgnn.pt')}")
    print("="*100)
...
if __name__ == "__main__":
    ...
    if training_mode == "full":
        ...
        print(f"\n use full model configuration ( best Performance ):")
        ...
        print(f" set2set_steps: {set2set_steps} ( from 3 reduce to 2, approximately 1.2-1.5 Double Acceleration )")
        ...
        print(f" set2set_steps: {set2set_steps} ( from 3 reduce to 2)")
        print(f" post_explain_layers: {post_explain_layers} ( from 2 reduce to 1)")
        print(f" note : Performance may be degraded 5-10%")
