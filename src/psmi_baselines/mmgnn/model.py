...
class IntramolecularMessagePassing(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, u, batch=None):
        ...
        if batch is not None:
            ...
            u_expanded = u.expand(edge_index.size(1), -1)
...
class Explainer(nn.Module):
    ...
    def __init__(self, node_dim: int, edge_dim: int, method: str = 'local_mask'):
        ...
        if method == 'local_mask':
            self.node_mask = nn.Parameter(torch.ones(1))
...
class MMGNN(nn.Module):
    ...
    def __init__(
        ...
        self.global_proj = nn.Linear(6, hidden_dim)
        ...
        predictor_input_dim = hidden_dim * 2 * 3 + 2
            ...
            if batch is not None:
                ...
                if u.size(0) == batch_size * 3 * 6:
                    ...
                    if u.size(0) % 6 == 0:
                        ...
                        raise ValueError(f"Unsupported one-dimensional global_features shape: {u.shape}")
            else:
                if u.size(0) % 6 == 0:
                    u = u.view(-1, 6)
                else:
        ...
        if batch is not None:
            ...
            if u.size(0) == batch_size * 3 and u.size(1) == 6:
                ...
                for b in range(batch_size):
                    ...
                    u_list.append(u_batch.mean(dim=0))
                ...
                if u.size(1) == 6:
                    ...
                    raise ValueError(f"Unsupported global_features shape {u.shape} for batch_size={batch_size}; the second dimension must be a multiple of 6.")
            else:
                if u.size(0) % batch_size == 0:
                    ...
                    if u.size(1) == 6:
                        ...
                        raise ValueError(f"Unsupported global_features shape {u.shape} for batch_size={batch_size}.")
                else:
                    raise ValueError(f"global_features length {u.size(0)} is not divisible by batch_size={batch_size}.")
        else:
