# -*- coding: utf-8 -*-
"""Implement the archived data.py compatibility module."""
...
def _norm_col(c: Any) -> str:
    """Run the legacy norm col operation."""
    # Legacy public-release workflow step.
...
def _find_col(available_cols: List[str], candidates: List[str]) -> Optional[str]:
    """Run the legacy find col operation."""
    # Legacy public-release workflow step.
...
def _require_col(df: pd.DataFrame, name: str, candidates: List[str]) -> str:
    """Run the legacy require col operation."""
...
def load_and_prepare_excel(
    ...
    def _req_comp(name: str) -> str:
        return _require_col(df, name, [name, name.upper(), name.lower(), name.replace("x", "X"), name.replace("X", "x")])
...
def stratified_split_by_system(
    ...
    """Run the legacy stratified split by system operation."""
...
def _cfg_to_namespace(cfg_module_or_obj: Any) -> Any:
    """Run the legacy cfg to namespace operation."""
